...
extern int ITK_user_main (int argc, char ** argv )
{
	...
	char*       Assembly=NULL;
	char*       changeTypeIn=NULL;
	char*       Finished=NULL;
	char*       InHouseStock=NULL;
	char*       InHousetTotal=NULL;
	char*       InShop=NULL;
	char*       IsPartDevIn=NULL;
	char*       MBPAStorelocIn=NULL;
	char*       OldQtyL=NULL;
	char*       Peculiarity=NULL;
	char*       PecuValid=NULL;
	char*       PoGoheadIn=NULL;
	char*       PrtSupplierIn=NULL;
	char*       RawParts=NULL;
	char*       ReqForMepaIn=NULL;
	char*       SampleIRMRIn=NULL;
	char*       SPD=NULL;
	char*       SupplierEnd=NULL;
	char*       Transit=NULL;
	char*       WareHouse=NULL;
...
//    ITK_CALL(ITK_init_module("APLloader","abc123","APLCAR"));
    ...
    strcpy(outputfile,"/home/cmitest/EPAMigration/EPAToInvalidPartException.log");
	...
	if(fp!=NULL)
	{
		...
		while(fgets(inputline,5000,fp)!=NULL)
		{
			...
			Assembly=strtok(NULL,"^");  //6
			printf("Assembly is --->%s\n",Assembly);
			changeTypeIn=strtok(NULL,"^");  //7
			printf("changeTypeIn is --->%s\n",changeTypeIn);
			Finished=strtok(NULL,"^");  //8
			printf("Finished is --->%s\n",Finished);
			InHouseStock=strtok(NULL,"^");  //9
			printf("InHouseStock is --->%s\n",InHouseStock);
			InHousetTotal=strtok(NULL,"^");  //10
			printf("InHousetTotal is --->%s\n",InHousetTotal);
			InShop=strtok(NULL,"^");  //11
			printf("InShop is --->%s\n",InShop);
			IsPartDevIn=strtok(NULL,"^");  //12
			printf("IsPartDevIn is --->%s\n",IsPartDevIn);
			MBPAStorelocIn=strtok(NULL,"^");  //13
			printf("MBPAStorelocIn is --->%s\n",MBPAStorelocIn);
			OldQtyL=strtok(NULL,"^");  //14
			printf("OldQtyL is --->%s\n",OldQtyL);
			Peculiarity=strtok(NULL,"^");  //15
			printf("Peculiarity is --->%s\n",Peculiarity);
			PecuValid=strtok(NULL,"^");  //16
			printf("PecuValid is --->%s\n",PecuValid);
			PoGoheadIn=strtok(NULL,"^");  //17
			printf("PoGoheadIn is --->%s\n",PoGoheadIn);
			PrtSupplierIn=strtok(NULL,"^");  //18
			printf("PrtSupplierIn is --->%s\n",PrtSupplierIn);
			RawParts=strtok(NULL,"^");  //19
			printf("RawParts is --->%s\n",RawParts);
			ReqForMepaIn=strtok(NULL,"^");  //20
			printf("ReqForMepaIn is --->%s\n",ReqForMepaIn);
			SampleIRMRIn=strtok(NULL,"^");  //21
			printf("SampleIRMRIn is --->%s\n",SampleIRMRIn);
			SPD=strtok(NULL,"^");  //22
			printf("SPD is --->%s\n",SPD);
			SupplierEnd=strtok(NULL,"^");  //23
			printf("SupplierEnd is --->%s\n",SupplierEnd);
			Transit=strtok(NULL,"^");  //24
			printf("Transit is --->%s\n",Transit);
			WareHouse=strtok(NULL,"^");  //25
			printf("WareHouse is --->%s\n",WareHouse);

			// Start for EPA and InValid parts relationship
			if(strcmp(TypeName,"EpaInValid")==0)
			{
			...
			if(n_tags_found_epa > 0  && resultCount>0)
			{
				...
				result = GRM_find_relation_type("T5_EPAHasInvalidPart", &relation_type);

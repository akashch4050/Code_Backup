...
extern int ITK_user_main (int argc, char ** argv )
{
	...
	char    *FileDown2,*FileUses2 = NULL,*File_LhRh,*File_Weld,*FilePath,*File_Pro,*File_Dml,*DepthInputStr,*type,*FileMissingDs;
	...
	char
		*qry_entries4[3] = {"creation_date_after","creation_date_before","Project Code"},
        *qry_values4[2]	= {"*"};

	//char* designGroupList="69";
	char* designGroupList="15,54,60,61,62,63,64,65,66,67,68,69,70,71,72,73,74,75,76,77,78,79,80,81,82,83,84,85,86,87,88,89,90,91,92,93,94,95,96,97,98,99";
	...
	if ( req_item )
    {
			...
			// Find the DML-Revision tag. 
				printf("\n\n Inside Design ! ! ! \n\n");  fflush(stdout);
					...
					temp1=MEM_alloc(25);
					//attrs[0] ="t5_DesignGrp";
					attrs[0] ="item_id";
					//values[0] = "5442*"
					tc_strcpy(temp1,"");
					tc_strcpy(temp1,req_item);
					tc_strcat(temp1,"*");
					printf("\n Temp=== %s",temp1);fflush(stdout);
					values[0] = temp1;

					attrs[1] ="object_type";
					values[1] = "Design";
					printf("\n\n Item project=%s \n\n",values[0]);  fflush(stdout);
					ITK_CALL(ITEM_find_items_by_key_attributes(2,attrs, values, &n_tags_found, &tags_found));
					printf ("ITEM_find_items_by_key_attributes found\n=%d",n_tags_found);
					MEM_free(attrs);
					MEM_free(values);
					if (n_tags_found == 0)
					{
				...
				/*	else if (n_tags_found > 1)
				   ...
				   if(QRY_find("DesRevFromDateToDateCatiaV6", &queryTag));
					printf("\n\t QRY_find DesRevFromDateToDateCatiaV6.... "); fflush(stdout);

					if (queryTag)
					{
						printf("Found Query DesRevFromDateToDateCatiaV6 \n"); fflush(stdout);
					}
					else
					{
						printf("Not Found Query DesRevFromDateToDateCatiaV6 \n"); fflush(stdout);
					...
					qry_values[1] = ToDate;
					qry_values[2] = req_item;

					ITK_CALL(QRY_execute(queryTag, n_entries, qry_entries4, qry_values, &resultCount, &rev));

					printf(" Qry===%d  \n",resultCount); fflush(stdout);

					for ( itemCount=0;itemCount<resultCount ;itemCount++ )
					{
					printf ("Upendra count=%d\n", itemCount);fflush(stdout);
					revLatest = rev[itemCount];
				
					//ITK_CALL(ITEM_ask_latest_rev (item_tag,&revLatest) );	
					if(revLatest) printf ("Latest Revision found\n");
					printf ("Upendra 1.112\n");fflush(stdout);	
					ITK_CALL(AOM_ask_value_string(revLatest,"item_id",&item_id));
					ITK_CALL(AOM_ask_value_string(revLatest,"t5_PartType",&t5_PartType));
					ITK_CALL(AOM_ask_value_string(revLatest,"t5_DesignGrp",&t5_DesignGrp));
					ITK_CALL(RES_is_checked_out	(rev[itemCount],&is_chkout));	
					...
					printf ("Upendra 1.000\n");fflush(stdout);	
					printf ("Upendra Design group list=%s\n",designGroupList);fflush(stdout);	
					printf(" 1] Design Group_strstr= %d\n\n\n",tc_strstr(t5_DesignGrp,designGroupList));


					if(tc_strcmp(t5_PartType,type)==0 && tc_strstr(designGroupList,t5_DesignGrp)!=NULL && is_chkout == 0)
						{	printf ("Upendra 1.111\n");fflush(stdout);					
							...
							ITK_CALL(BOM_set_window_top_line (window, item_tag, revLatest, null_tag, &top_line));
							print_bom (top_line,NULLTAG, 0,fd,fu,fLhRh,fweld,fpro,req_item,FilePath,DepthInput,fmissingds);
						}
						printf ("Upendra 1.444\n");fflush(stdout);	
					}
...
//	int    Item_ID,Item_Revision,Item_Description,parent_item_seq=0,QuantityAttrId,TransforationMatrixAttrId,Item_WeldPrts,PrtTypeAtr,iChildItemTag;
	...
	char   *Weld_Item;
	char   pathname[SS_MAXPATHLEN + 1];
	char   pathnamedir[SS_MAXPATHLEN + 1];
	char   pathnameJT[SS_MAXPATHLEN + 1];
	char   pathnamePdf[SS_MAXPATHLEN + 1];
	char   *ent_seq_str;
	char   *parentrev=NULL;
...
//									}
									...
									STRNG_replace_str(Item_Revision_str_O,",","_",&Item_Revision_str_2);
									sprintf(pathnamedir,"mkdir %s%s/%s_%s",FilePath,req_item,Item_ID_str,Item_Revision_str_2);
									printf("\n pathnamedir=%s\n",pathnamedir);
									system(pathnamedir);
									sprintf(pathname,"%s%s/%s_%s/%s",FilePath,req_item,Item_ID_str,Item_Revision_str_2,orig_name_Tmp );

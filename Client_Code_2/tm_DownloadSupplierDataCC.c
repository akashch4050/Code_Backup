...
static tag_t ask_item_revision_from_bom_line_ActHndlr(tag_t bom_line)
{
	...
	ITK_CALL(AOM_ask_value_tag(bom_line, "bl_line_object", &item_revision));
								...
								)
{
	...
	char* PartDesc = NULL;
	char* part_app_own = NULL;
	char* PartAppOwn = NULL;
	char* FileNameStr = NULL;
	//char FileNameStr[IMF_filename_size_c + 1];
	char* OutputPartDesc = NULL;
	char jtFname[100];
	FILE* jtfp = NULL;
	int jtFileFlag = 0;
	int CadFileFlag = 0;
	...
	tag_t *GenInsTags	= NULLTAG;
	tag_t IPEMRelTag	= NULLTAG;
	tag_t IMANSpecRelTag = NULLTAG;
	tag_t CreoGenRelTag = NULLTAG;
	tag_t InsRefobject = NULLTAG;
	char GenInst_name[TCTYPE_name_size_c+1] ;
	char* CreoGenId = NULL;
	char InsItemId[IMF_filename_size_c + 1];
	char* InsReftype = NULL;
	char* svrStr	= NULL;
	int	n_genIns = 0,n_InsCnt=0,InsNamedRefCount=0,k=0;
	...
	//tc_strcpy(jt_orig_name,"NoJTExist");
	FileNameStr=(char *) MEM_alloc(10000 * sizeof(char ));
	tc_strcpy(FileNameStr,"");
	...
	for (iFmtCnt = 0; iFmtCnt < cadTyp_num_val  ;iFmtCnt++ )
	{
		...
		{
			ITK_CALL(AOM_ask_value_string(*item_rev_tags,"t5_ApplicationOwner",&PartAppOwn));
			printf("\nPart Application Owner [%s]",PartAppOwn); fflush(stdout);
		}
		...
		if (jtFmtFlag == 1)
		{
			ITK_CALL(BOM_line_ask_absocc_relation (bom_line, NULLTAG, "IMAN_Rendering", TRUE, &n_Alfrel_item, &Alfrel_items));
			printf("\nTotal BOM_line_ask_absocc_relation n_Alfrel_item: %d\n",n_Alfrel_item); fflush(stdout);
			...
			if (n_Alfrel_item > 0)
			{
				...
				for(j=0;j<n_Alfrel_item;j++)
				{
					...
					printf("\nBomLine Dataset Reference Count %d\n", referencenumberfound); fflush(stdout);
					for(n_Alfrel_item=0;n_Alfrel_item<referencenumberfound;n_Alfrel_item++)
					{
						ITK_CALL(AE_find_dataset_named_ref(datasetAlf,n_Alfrel_item,refname,&reftype,&refobject));
						ITK_CALL(IMF_ask_original_file_name(refobject,Alforig_name));
						printf("\nALF orig_name is : %s",Alforig_name); fflush(stdout);
						jtFileFlag = 1;
						tc_strcat(jt_orig_name,Alforig_name);
						tc_strcat(jt_orig_name,";");
						printf("\nALF_jt_orig_name : %s",jt_orig_name);fflush(stdout);
						tc_strcat(CadDataDownloadDir,Alforig_name);

						printf("\nDownloading...:%s",CadDataDownloadDir);fflush(stdout);
						IMF_export_file(refobject,CadDataDownloadDir);	//Downloading ALF/WELD JT
					}
				}
			}
		}
		...
		for (i= 0; i < n_attchs; i++)
		{
			...
			if(
				...
				(tc_strcmp(type_name,"CMI2AuxPart")==0) ||
				(tc_strcmp(type_name,"CMI2Drawing")==0) ||
				(tc_strcmp(type_name,"CMI2Cgr")==0) ||
				(tc_strcmp(type_name,"CMI2CacheCgr")==0) ||
			...
			)
			{
				...
				for(j = 0 ;j < referencenumberfound; j++)
				{
					...
					{
						printf("\nSkipping_0 as not iman file:%s",type);fflush(stdout);
						continue;
					}
					if(tc_strcmp(type,"CMI2BoundingBoxForm")==0)
					{
						printf("\nSkipping_1a:%s",type);fflush(stdout);
						continue;
					}
					else if(tc_strcmp(type,"CMI2AuxFileInfoForm")==0)
					{
						printf("\nSkipping_2a:%s",type);fflush(stdout);
						continue;
					}
					else if(tc_strcmp(type,"CMI2TypeForm")==0)
					{
						printf("\nSkipping_3a:%s",type);fflush(stdout);
						continue;
					}
					...
					if(tc_strcmp(DownloadFormatList[iFmtCnt],"JTVis")==0)
					{
						...
						else
						{
							...
							{
								jtFileFlag = 1;
								tc_strcat(jt_orig_name,orig_name);
								tc_strcat(jt_orig_name,";");

								printf("\njt_orig_name : %s",jt_orig_name);fflush(stdout);
							}
						}
					...
					{
						printf("\ntype_name_0:%s",type_name);fflush(stdout);
						printf("\ntype_2:%s",type);fflush(stdout);
						//CMI2Part	CATPart
						...
						if(
							...
							&& (tc_strstr(type_name,"CMI2AuxPart")==NULL)
					...
					else if(tc_strcmp(DownloadFormatList[iFmtCnt],"CAD2D")==0)
					{
						...
						printf("\ntype_name_1:%s",type_name);fflush(stdout);
						printf("\ntype_1:%s",type);fflush(stdout);
				...
				printf("\n==========================start================================");fflush(stdout);
				printf("\nreferencenumberfound_1:%d",referencenumberfound);fflush(stdout);
				for(j = 0 ;j < referencenumberfound; j++)
				{
					printf("\nj:%d",j);fflush(stdout);
					ITK_CALL(AE_find_dataset_named_ref(primary,j,refname,&reftype,&refobject));
					ask_property_value_by_name_ActHndlr(refobject, "Type", &type);
					printf("\nGetting File Name As per Application Owner; File Ref Type:%s",type);//CMI2TypeFormfflush(stdout);
					
					if(tc_strcmp(type,"ImanFile")!=0)
					{
						printf("\nSkipping_00_a as no ImanFile:%s",type);fflush(stdout);
						continue;
					}
					if(tc_strcmp(type,"CMI2BoundingBoxForm")==0)
					{
						printf("\nSkipping_11:%s",type);fflush(stdout);
						continue;
					}
					else if(tc_strcmp(type,"CMI2AuxFileInfoForm")==0)
					{
						printf("\nSkipping_22:%s",type);fflush(stdout);
						continue;
					}
					else if(tc_strcmp(type,"CMI2TypeForm")==0)
					{
						printf("\nSkipping_33:%s",type);fflush(stdout);
						continue;
					}

					ITK_CALL( IMF_ask_original_file_name(refobject,orig_name));
					printf("\nFile Orignal_Name:[%s]",orig_name);fflush(stdout);
					
					if(tc_strlen(PartAppOwn)==0) 
					{
						if(tc_strcmp(type_name,"ProDrw")==0)
						{
							printf("\nskipping_b1 type_name:%s",type_name);
							continue;
						}
						if(tc_strcmp(type_name,"CMI2Drawing")==0)
						{
							printf("\nskipping_b2 type_name:%s",type_name);
							continue;
						}
						if(tc_strcmp(type_name,"ProDrw")==0)
						{
						...
						//tc_strcpy(FileNameStr,"");
						if((tc_strstr(type_name,"CMI2Part")!=NULL) || 
						(tc_strstr(type_name,"CMI2AuxPart")!=NULL) ||
						(tc_strstr(type_name,"CMI2Product")!=NULL) ||
						(tc_strstr(type_name,"ProAsm")!=NULL) || 
						(tc_strstr(type_name,"ProPrt")!=NULL))
						{
							if((strstr(orig_name,".asm.")!=NULL) || (strstr(orig_name,".prt.")!=NULL) || (strstr(orig_name,".CATPart")!=NULL) || (strstr(orig_name,".CATProduct")!=NULL))
							{
								tc_strcat(FileNameStr,orig_name);
								tc_strcat(FileNameStr,";");
							}
						}
						continue;
					}
					/*else if((tc_strcmp(PartAppOwn,"Cat-Part")==0) && (tc_strstr(type_name,"CMI2Part")!=NULL))
					{
						if((strstr(orig_name,".CATPart")!=NULL) || (strstr(orig_name,".CATProduct")!=NULL))
						{
							//tc_strcpy(FileNameStr,"");
							tc_strcat(FileNameStr,orig_name);
							tc_strcat(FileNameStr,";");
					...
					else if((tc_strcmp(PartAppOwn,"Cat-Product")==0) && (tc_strstr(type_name,"CMI2Product")!=NULL))
					{
						if((strstr(orig_name,".CATPart")!=NULL) || (strstr(orig_name,".CATProduct")!=NULL))
						{
							//tc_strcpy(FileNameStr,"");
					...
					else if((tc_strcmp(PartAppOwn,"ProEAsm")==0) && (tc_strstr(type_name,"ProAsm")!=NULL))
					{
						
						if((strstr(orig_name,".asm.")!=NULL) || (strstr(orig_name,".prt.")!=NULL))
						{
							//tc_strcpy(FileNameStr,"");
							tc_strcat(FileNameStr,orig_name);
							tc_strcat(FileNameStr,";");
					...
					else if((tc_strcmp(PartAppOwn,"ProEPart")==0) && (tc_strstr(type_name,"ProPrt")!=NULL))
					{
						if((strstr(orig_name,".asm.")!=NULL) || (strstr(orig_name,".prt.")!=NULL))
						{
							//tc_strcpy(FileNameStr,"");
					...
					if(
						((tc_strcmp(PartAppOwn,"Cat-Part")==0) || (tc_strcmp(PartAppOwn,"Cat-Product")==0))
						&& 
						((tc_strstr(type_name,"CMI2Part")!=NULL) || (tc_strstr(type_name,"CMI2AuxPart")!=NULL) || (tc_strstr(type_name,"CMI2Product")!=NULL))
						)
					{
						if((strstr(orig_name,".CATPart")!=NULL) || (strstr(orig_name,".CATProduct")!=NULL))
						{
						...
						((tc_strcmp(PartAppOwn,"ProEAsm")==0) || (tc_strcmp(PartAppOwn,"ProEPart")==0))
						&& 
						((tc_strstr(type_name,"ProAsm")!=NULL) || (tc_strstr(type_name,"ProPrt")!=NULL))
						)
					{
						if((strstr(orig_name,".asm.")!=NULL) || (strstr(orig_name,".prt.")!=NULL))
						{
				...
				printf("\nj:%d Type_Name_11	[%s]	FileNameString [%s]",j,type_name,FileNameStr);fflush(stdout);
				printf("\n==========================end================================");fflush(stdout);
			}

			//Generic Instance Logic
			//Creo part and Creo assembly
			if(tc_strcmp(type_name,"ProPrt")==0 || tc_strcmp(type_name,"ProAsm")==0)
			{
				ITK_CALL(AE_ask_dataset_ref_count(primary,&referencenumberfound));
				printf("\nGeneric Instance Ref Num Found : %d",referencenumberfound);fflush(stdout);

				if(referencenumberfound==0)
				{
					printf("\nInside relation : IPEM_master_dependency\n");fflush(stdout);
					ITK_CALL(GRM_find_relation_type("IMAN_specification",&IMANSpecRelTag));
					ITK_CALL(GRM_find_relation_type("IPEM_master_dependency",&IPEMRelTag ));
					ITK_CALL(TCTYPE_ask_name(IPEMRelTag,GenInst_name));
					
					ITK_CALL(GRM_list_secondary_objects_only(primary,IPEMRelTag,&n_genIns,&CreoGenTags));
					printf("\nGeneric Instance GenInst_name : %s and n_genIns : %d",GenInst_name,n_genIns);
					for (j=0;j<n_genIns;j++)
					{
						ITK_CALL(AOM_UIF_ask_value(CreoGenTags[j],"item_id",&CreoGenId));
						printf("\nGeneric Instance CreoGenId : %s",CreoGenId);fflush(stdout);

						ITK_CALL(GRM_list_secondary_objects_only(CreoGenTags[j],IMANSpecRelTag,&n_InsCnt,&GenInsTags));
						
						printf("\nGeneric Instance IMAN_specification Objects Found : %d",n_InsCnt);fflush(stdout);

						for (k=0;k<n_InsCnt;k++)
						{
							ITK_CALL(AE_ask_dataset_ref_count(GenInsTags[k],&InsNamedRefCount));
							printf("\nInsNamedRefCount : %d",InsNamedRefCount);fflush(stdout);
							if (InsNamedRefCount>0)
							{
								ITK_CALL(AE_find_dataset_named_ref(GenInsTags[k],0,refname,&reftype,&InsRefobject));
								ITK_CALL(IMF_ask_original_file_name(InsRefobject,InsItemId));
								printf("\nInsItemId : %s",InsItemId);fflush(stdout);

								tc_strcpy(CadDataDownloadDir,"");
								tc_strcpy(CadDataDownloadDir,PartSubDirectoryNameFinal);
								tc_strcat(CadDataDownloadDir,"/");
								tc_strcat(CadDataDownloadDir,InsItemId);

								printf("\nDownloading...:%s",CadDataDownloadDir);fflush(stdout);
								
								IMF_export_file(InsRefobject,CadDataDownloadDir);

								tc_strcpy(FileNameStr,InsItemId);
								break;
							}
						}

						if (tc_strlen(InsItemId)!=0)
						{
							break;
						}
					}
		...
		//If JT file not Found Flag
		if(tc_strcmp(DownloadFormatList[iFmtCnt],"JTVis")==0 && jtFileFlag ==0)
		{
			//If JT file not Found Flag
			printf("\nJT file not Found Searching for CAD Files jtFileFlag : %d ",jtFileFlag);fflush(stdout);
			
			if (n_attchs==0)
			{
				tc_strcat(jt_orig_name,"NoCADExist");
			}

			for (i= 0; i < n_attchs; i++)
			{
				printf("\nFile num i:%d",i);fflush(stdout);
				//primary=rellist[i].secondary;
				primary=secondary_objects[i];
				
				ITK_CALL(TCTYPE_ask_object_type(primary,&objTypeTag));
				ITK_CALL(TCTYPE_ask_name(objTypeTag,type_name));
				printf("\tFile Type Name:%s",type_name);fflush(stdout);
				...
				if( tc_strcmp(type_name,"CMI2Product")==0 ||
					tc_strcmp(type_name,"CMI2Part")==0 ||
					tc_strcmp(type_name,"CMI2AuxPart")==0 ||
					tc_strcmp(type_name,"Creo assembly")==0 ||
					tc_strcmp(type_name,"Creo part")==0 ||
					tc_strcmp(type_name,"ProAsm")==0 ||
					tc_strcmp(type_name,"ProPrt")==0
				)
				{
					CadFileFlag = 1;
					tc_strcat(jt_orig_name,"NoJTExist");
					break;
				}
				else
				{
	...
	if(jtFmtFlag == 1)
	{
		printf("\nApplication Owner [%s]	FileName [%s]\n",PartAppOwn,FileNameStr);fflush(stdout);
		...
		ITK_CALL(AOM_ask_value_string(*item_rev_tags,"object_type",&PartObjType));
		ITK_CALL(AOM_UIF_ask_value(*item_rev_tags,"release_status_list",&PartRelStat));
		ITK_CALL(AOM_ask_value_string(*item_rev_tags,"object_desc",&PartDesc));
		
		if(tc_strcmp(PartObjType,"Design Revision")==0)
		{
			ITK_CALL(AOM_ask_value_string(*item_rev_tags,"t5_PartType",&sPartType));
			ITK_CALL(AOM_ask_value_string(*item_rev_tags,"t5_ApplicationOwner",&part_app_own));
		}

		if (tc_strlen(PartDesc)==0)
		{
			tc_strdup("NULL_DESC",&PartDesc);
		}

		if(tc_strlen(part_app_own)==0)
		{
			tc_strdup("ApplOnwerBLANK",&part_app_own);
		}

		printf("\nitem_id [%s]	oringnal object_desc [%s]\n",item_id,PartDesc);fflush(stdout);
		//STRNG_replace_str(const char * 	input,const char * 	strToBereplaced,const char * 	strToBeUsedInstead,char ** 	output )
		STRNG_replace_str(PartDesc,",","_",&PartDesc);
		STRNG_replace_str(PartDesc,".","_",&PartDesc);
		STRNG_replace_str(PartDesc,";","_",&PartDesc);
		STRNG_replace_str(PartDesc,"-","_",&PartDesc);
		STRNG_replace_str(PartDesc,"^","_",&PartDesc);
		...
		printf("\nitem_id [%s]	after skip wild char PartDesc [%s]\n",item_id,PartDesc);fflush(stdout);
		...
		ITK_CALL(AOM_ask_value_string(bom_line, "bl_formatted_parent_name", &ParentPart));
		ParentNum = strtok (ParentPart, "/");

		if (((*depth)-1==0) && SSFlag == 1 && tc_strcmp(PartObjType,"T5_PlatformRevision")==0)
		{
			//Copying SVR Number instead X4 for firts level
			svrStr	= (char *) MEM_alloc(50 * sizeof(char ));
			tc_strcpy(svrStr,strtok(SVRNumber,"_"));
			tc_strcat(svrStr,"_");
			tc_strcat(svrStr,strtok(NULL,"_"));

			tc_strdup(svrStr,&item_id);

			printf("\nCopying SVR Number instead X4 SVRNumber [%s]\n",item_id);
		}

		printf("%d^%s^%s^%s^%s^%s^%s^%d^%s^%s^%s^%s^%s^%s^\n",(*depth)-1,item_id,newPatRev,newPatSeq,sPartType,jt_orig_name,PartRelStat,iIs_Suppressed,RelTransMatrix,part_app_own,FileNameStr,PartDesc,PartObjType,ParentNum);fflush(stdout);
		fprintf(jtfp,"%d^%s^%s^%s^%s^%s^%s^%d^%s^%s^%s^%s^%s^%s^\n",(*depth)-1,item_id,newPatRev,newPatSeq,sPartType,jt_orig_name,PartRelStat,iIs_Suppressed,RelTransMatrix,part_app_own,FileNameStr,PartDesc,PartObjType,ParentNum);
		if(jtfp!=NULL)
		{
			fclose(jtfp);
		}
								...
								)
{
	...
	char *BOMJTFileNamePath = NULL;
	char *DatasetFileName = NULL;
	char *BOMJTFileName = NULL;
    ...
    tag_t file = NULLTAG;
    tag_t JTFile = NULLTAG;
	IMF_file_t descriptor;
	IMF_file_t JTFileDescriptor;
	tag_t jtfiledataset = NULLTAG; 
	FILE* datasetfp = NULL;
	tag_t query = NULLTAG;
	int n_entries	= 3;
   ...
   char *qry_values[3];
	...
	else
	{
		...
		ITK_CALL(AOM_ask_value_string(*FolderobjTag,"t5_PackTitle",&Date_Eff_Str));

		printf("\nexp View :%s",t5_ViewName);fflush(stdout);
		printf("\nexp CfgContext :%s",t5_CfgContext);fflush(stdout);
		printf("\nDate Effective From I :%s",Date_Eff_Str);fflush(stdout);	//DateTime {5/7/2019}

		STRNG_replace_str(Date_Eff_Str,"DateTime {","",&Date_Eff_Str);
		STRNG_replace_str(Date_Eff_Str,"}","",&Date_Eff_Str);

		printf("\nDate Effective From F :%s",Date_Eff_Str);fflush(stdout);

		//DATE_convert_formatted_string_to_date(const char * date,const char * format,logical localized_default_format,logical stringent_check,date_t * date_structure)
		ITK_CALL(DATE_convert_formatted_string_to_date(Date_Eff_Str,"%m/%d/%Y",false,true,&Date_Eff));

		ITK_CALL(ITK_date_to_string(Date_Eff,&Date_Eff_Str));

		printf("\nConverted Effective Date :%s",Date_Eff_Str);fflush(stdout);

		//ITK_CALL(ITK_string_to_date(Date_Eff_Str,&Date_Eff));	//07-May-2019 10:40
		...
		else
		{
			...
			if(SSFlag==1)
			{
				printf("\n\n*****Setting BOM_create_window_from_snapshot*****\n");fflush(stdout);
				ITK_CALL(BOM_create_window_from_snapshot(Snapshot_tag,&window));
			}
			else
			{
				ITK_CALL(CFM_find( CxtRevRuleName, &rule ));
				printf("\nsetting CxtRevRuleName:%s",CxtRevRuleName);
				//ITK_CALL(CFM_find( "ERC release and above", &rule ));
				ITK_CALL(CFM_rule_set_date(rule,Date_Eff));
				printf("\nsetting Date_Eff_Str:%s",Date_Eff_Str);
				...
				} /*added by Devkant 15.01.2019*/
				else if( tc_strcmp(t5_ViewName,"ERC") == 0 )
				{
					ITK_CALL(BOM_set_window_config_rule( window, rule ));
					ClosureRule = (char *) MEM_alloc(100 * sizeof(char ));
					tc_strcpy(ClosureRule,"BOMViewClosureRuleERC");
					printf("\nClosureRule ERC:%s",ClosureRule);
					ITK_CALL(PIE_find_closure_rules( ClosureRule,PIE_TEAMCENTER, &n_closure_tags, &closure_tags ));
					printf("\nn_closure_tags123 ERC:%d ..............",n_closure_tags);fflush(stdout);
					if(n_closure_tags > 0)
					{
						closure_tag=closure_tags[0];
						ITK_CALL(BOM_window_set_closure_rule(window,closure_tag,0,NULL,NULL));
			...
			BOMJTFileNamePath = (char*) MEM_alloc(200*sizeof( char ));
			BOMJTFileName = (char*) MEM_alloc(200*sizeof( char ));
			...
			if (jtFmtFlag == 1)
			{
				//Attaching BOM File to Package
				printf("\n\nAttaching BOM File to Package : %s/%s/%s/BOMList_JTFiles_%s.txt",SupplierDataDir,usernameDir,SubDirectoryName,PartSubDirectoryName);

				tc_strcpy(BOMJTFileNamePath,SupplierDataDir);
				tc_strcat(BOMJTFileNamePath,"/");
				tc_strcat(BOMJTFileNamePath,usernameDir);
				tc_strcat(BOMJTFileNamePath,"/");
				tc_strcat(BOMJTFileNamePath,SubDirectoryName);
				tc_strcat(BOMJTFileNamePath,"/BOMList_JTFiles_");
				tc_strcat(BOMJTFileNamePath,PartSubDirectoryName);
				tc_strcat(BOMJTFileNamePath,".txt");

				tc_strcpy(BOMJTFileName,"BOM_JTFiles_List");
				tc_strcat(BOMJTFileName,PartSubDirectoryName);
				tc_strcat(BOMJTFileName,".txt");

				printf("\nBOMJTFileNamePath : %s\nBOMJTFileName : %s",BOMJTFileNamePath,BOMJTFileName);fflush(stdout);

				ITK_CALL(AE_create_dataset_with_id(ds_type, BOMJTFileName, "BOM_JTFiles_List", BOMJTFileName, "NR", &jtfiledataset));
				ITK_CALL(AE_set_dataset_tool( jtfiledataset, tool));
				ITK_CALL(AE_set_dataset_format(jtfiledataset, "TEXT_REF"));
				ITK_CALL(AOM_save(jtfiledataset));

				ITK_CALL(IMF_import_file(BOMJTFileNamePath, BOMJTFileName, SS_TEXT,&JTFile, &JTFileDescriptor));
				ITK_CALL(AOM_save(JTFile));

				ITK_CALL(AOM_refresh(jtfiledataset, TRUE));
				ITK_CALL(AOM_lock(jtfiledataset));
				
				ITK_CALL(AE_add_dataset_named_ref(jtfiledataset, "Text", AE_PART_OF,JTFile));
				ITK_CALL(AE_save_myself(jtfiledataset));
				ITK_CALL(AOM_unlock(jtfiledataset));
				
				ITK_CALL(FL_insert(*FolderobjTag, jtfiledataset, -1));
				ITK_CALL(AOM_save(*FolderobjTag));
			}
		}
	}
...
//	struct tm *timeinfo;
	...
	printf("\nSession Username: %s",userId);fflush(stdout);
	printf("\nCreate_SupplierDirCC VendorName: %s",VendorName);fflush(stdout);
...
//	strftime(timestamp, 20, "%Y-%m-%d-%H_%M_%S", timeinfo);
	...
	printf("\nUsernameDir:%s",usernameDir);fflush(stdout);
...
callt5DataDownLoadMechCC(tag_t* objTag,FILE** fp,int* i,char* DwnloadFrmtSelctStr)
{
	...
	tc_strdup(userId,&DownLoadedBy);
	printf("\nDownLoadedBy userid1:%s",DownLoadedBy);
	...
	printf("\nDownLoadedBy username1:%s",username);
	...
	tok1 = NULL;
	tc_strcpy(partarr,"");
	printf("\nPartnumber5:%s",PartnumberList[*i]);fflush(stdout);
	...
	values[0]= partarr;
	values[1] = "Design";
	ITK_CALL(ITEM_find_items_by_key_attributes(2,attrs, values, &count, &tags));
	...
	if( count > 0 )
	{
		...
		tc_strcat(WriteLine,":;:TML_LOCAL_SHARE:>:msdcad_sys:>:msdcad_sysy1:>:animeshm.ttl@tatamotors.com:>::;::;:");
...
callJtFileShellCC(char* VendorName,char* inputline,char* SupplierDataDir,char* usernameDir,char* timestamp)
{
	...
	//tc_strcat(JtShellCommandLine,"/CollectJTFiles_IND_Relative_PLM_QA.sh");
...
int DownloadSupplierData_Func(char* PackageNum)
{
	...
	if(count2 > 0)
	{
		...
		if(tc_strcmp(ObjTyp,"T5_Package")== 0 )
		{
			...
			printf("\nDownLoadTypefn:%s",DownLoadType); //	char *t5_ViewName = NULL;
			...
			ITK_CALL(AOM_ask_value_string(objTag,"t5_ViewName",&ConfigView));
			printf("\nConfig View Name :%s",ConfigView);fflush(stdout);
			...
			if(num_values > 0)
			{
				...
				//ITK_CALL(POM_get_user_id(&DownLoadedBy));
				tc_strdup(userId,&DownLoadedBy);
				printf("\nDownLoadedBy userid2:%s:",DownLoadedBy);
				printf("\nusernameDir1234:%s:",usernameDir);
				...
				//ITK_CALL(POM_get_user(&username,&user_tag));
				printf("\nDownLoadedBy username2:%s",username);
				...
				tc_strcpy(SSCriteria.name,partarr);
				
				ITK_CALL(WSOM_search(SSCriteria,&SScount,&SSobjTag));
				...
				if (SScount>0)
				{
					...
					ITK_CALL(AOM_ask_value_tag(Snapshot_tag,"topLine",&item_rev_tags1));
					tc_strdup(partarr,&SVRNumber);
				...
				else
				{
					...
					values[0]= partarr;
					values[1] = "Design";
					ITK_CALL(ITEM_find_items_by_key_attributes(2,attrs, values, &count1, &tags1));
					//ITEM_ask_latest_rev(tags1[0],&item_rev_tags1);
				
					ITK_CALL(ITEM_find_revision(tags1[0],atok1,&item_rev_tags1));
				}

				ITK_CALL(AOM_UIF_ask_value(item_rev_tags1,"item_id",&newPartID));
				...
				printf("\nPartnumberList1[0]:%s",newPartID);fflush(stdout);
				...
				tc_strcat(fInputFileName,"_");
				tc_strcat(fInputFileName,newPartID);
				tc_strcat(fInputFileName,"_");
				...
				tc_strcpy(fDpdsWebFile,"");
				/*tc_strcpy(fDpdsWebFile,SupplierDataDir);
				tc_strcat(fDpdsWebFile,"/");
				tc_strcat(fDpdsWebFile,usernameDir);
				tc_strcat(fDpdsWebFile,"/");
				tc_strcat(fDpdsWebFile,SubDirectoryName);
				tc_strcat(fDpdsWebFile,"/");*/
				tc_strcpy(fDpdsWebFile,"/tmp/");
				tc_strcat(fDpdsWebFile,DownLoadedBy);
				tc_strcat(fDpdsWebFile,"_WebServiceFile_");
				tc_strcat(fDpdsWebFile,timestamp);
				tc_strcat(fDpdsWebFile,".txt");

				webf = fopen(fDpdsWebFile,"a");
				...
				for (i = 0; i < num_values; i++)
				{
					...
					if( count > 0 || SSFlag==1)
					{
						...
						ITK_CALL(AOM_UIF_ask_value(item_rev_tags,"item_id",&newPartID));
						ITK_CALL(AOM_UIF_ask_value(item_rev_tags,"item_revision_id",&cRevSeq2));
						ITK_CALL(AOM_UIF_ask_value(item_rev_tags,"owning_user",&iPartowner));
						ITK_CALL(AOM_ask_value_string(item_rev_tags,"object_desc",&iPartDesc));
						
						if (SSFlag==0)
						{
							ITK_CALL(AOM_ask_value_string(item_rev_tags,"t5_PartType",&iPartType));
						}
						else
						{
							tc_strdup("Snapshot",&iPartType);
						}
						...
						fprintf(webf,"%s^%s^%s^%s^%s^%s^%s^%s^%s^%s^%s^%s/%s/%s\n",newPartID,newPatRev2,newPatSeq2,DwnloadFrmtSelctStr,iPartType,VendorName,userId,iPartowner,iPartDesc,cnfgContext,ConfigView,SupplierDataDir,usernameDir,SubDirectoryName);fflush(stdout);
						...
						SendNotifyMail(VendorName,inputline,PackageNum,SupplierDataDir,usernameDir,timestamp);
					...
					fclose(webf);				
					//Calling Web Service S-Collab Start
					tc_strcpy(CommandLine,"");
					//tc_strcpy(CommandLine,"/user/plmsap/PLMSAP/apn/SupDataDownload/UpdateScollab.sh");
					tc_strcpy(CommandLine,"/user/cattstk/TCUA_DIGIBUCK/CreateJTAssy/UpdateScollab.sh");
					//tc_strcpy(CommandLine,SupplierDataShell);
					//tc_strcat(CommandLine,"/UpdateScollab.sh");
					tc_strcat(CommandLine," ");
					tc_strcat(CommandLine,fDpdsWebFile);
					printf("\nCommandLine:%s",CommandLine);fflush(stdout);
					system(CommandLine);
					printf("\nSystem command executed");fflush(stdout);
					//Calling Web Service S-Collab End
			}
			
			printf("\nData Download Completed Successfuly On Path : %s/%s/ !!!\n",SupplierDataDir,usernameDir);fflush(stdout);
		}
	}
	if(fp!=NULL)
	{
...
SendNotifyMail(char *VendorName,char *inputline,char *PackageNum,char *SupplierDataDir,char *usernameDir,char *timestamp)
{
	...
	{
		tc_strdup(userId,&username);
		tc_strdup("amoln.ttl@tatamotors.com",&user_email);
	}
	else
	{
		ITK_CALL(SA_ask_user_person_name2(user_tag,&username));
	...
	//sprintf(path,"%s/%s/%s_%s_%s",SupplierDataDir,usernameDir,VendorName,inputline,timestamp);
	sprintf(path,"%s/%s/",SupplierDataDir,usernameDir);
	printf("\nPath : %s",path);

	tc_strcpy(mail_subject,"Data Download Completed For ");
	tc_strcat(mail_subject,PackageNum);

	tc_strcpy(mail_body,"Package : ");
	tc_strcat(mail_body,PackageNum);
	tc_strcat(mail_body,"\nDownload Path : ");
	tc_strcat(mail_body,path);
...
extern int ITK_user_main (int argc, char ** argv )
{
	...
	userId	= ITK_ask_cli_argument("-h=");
	//usernameDir = ITK_ask_cli_argument("-h=");

	userId[tc_strlen(userId)-1] = '\0';
	
	tc_strdup(userId,&usernameDir);
	tc_strcat(usernameDir,"_tcua");

	printf("\nInput Package Number:%s:\n",InpPackage); fflush(stdout);
	printf("\nSession User : [%s]	UserIdLen : [%d]	usernameDir [%s]\n",userId,tc_strlen(userId),usernameDir); fflush(stdout);

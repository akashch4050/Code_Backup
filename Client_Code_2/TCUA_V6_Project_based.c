	...
	if ( req_item )
    {
			...
			// Find the DML-Revision tag. 
				...
				/*	else if (n_tags_found > 1)
					...
					for ( itemCount=0;itemCount<n_tags_found ;itemCount++ )
					{
					printf ("Upendra count=%d\n", itemCount);fflush(stdout);
					item_tag = tags_found[itemCount];
					...
					if(tc_strcmp(t5_PartType,type)==0 && tc_strstr(designGroupList,t5_DesignGrp)!=NULL)

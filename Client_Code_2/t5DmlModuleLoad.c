...
int Complete_DML_Download(char* DmlIdS, char* filenameS)
{
	...
	if(QRY_execute(queryTag, n_entries, qry_entries, qry_values, &resultCount, &DMLTAGS));
	if (resultCount >0)
	{
		printf("\n\n DML found ! ! ! \n\n");  fflush(stdout);
		dml_rev_tag=DMLTAGS[0];
	}
	else
	{
		printf("\n\n DML not found ! ! ! \n\n");  fflush(stdout);
	...
	if (dml_rev_tag != NULLTAG)
	{
		...
		if (dml_rev_tag  != NULLTAG)
		{
			...
			STRNG_replace_str(object_nameS_O,"\n"," ",&object_nameS_OL);
			printf("\n Complete_DML_Download : object_nameS : =%s\n",object_nameS_O);
			if (object_nameS_OL)               
				strcat(AllDmlAttrS,object_nameS_OL);
			strcat(AllDmlAttrS,",");
			...
			if (AOM_ask_value_string(dml_rev_tag,"t5_PlntToPlnt",&DrStsChange)!=ITK_ok)   PrintErrorStack();
			printf("\n Complete_DML_Download : DR St : %s \n",DrStsChange); fflush(stdout);
			if (DrStsChange)strcat(AllDmlAttrS,DrStsChange);
			strcat(AllDmlAttrS,",");

			//20 Aggregate t5_ERCAggregate
			int num_Agg=0;
			int IAgg=0;
			char **AggList;
			if (AOM_ask_value_strings(dml_rev_tag,"t5_ERCAggregate",&num_Agg,&AggList)!=ITK_ok)   PrintErrorStack();
			printf("\n Complete_DML_Download : t5_ERCAggregate  , %d\n",num_Agg); fflush(stdout);
			for (IAgg=0;IAgg<num_Agg;IAgg++ )
			{
				if (IAgg>0)
				{
					strcat(AllDmlAttrS,"~");
				...
				strcat(AllDmlAttrS,AggList[IAgg]);
			...
			if (ClosureTmStmp!=NULL)
			{
				...
				printf("\nClosureTmStmpFinal----> %d-%d-%d:%d:%d:%d\n",CTSday,CTSmonth,CTSyear,CTShour,CTSminute,CTSsecond);
				CTSmonth++;
			...
			if (reln_type_tag	!=	NULLTAG)
			{
				...
				for (i=0;i<n_tasks ;i++ )
				{
					...
					if (task_rev_tag  != NULLTAG)
					{
						...
						if (tc_strcmp(DMLtype,"MR")==0 || tc_strcmp(DMLtype,"Veh")==0)
						{
							ITK_CALL(GRM_find_relation_type("CMHasSolutionItem",&reln_type_tag));
						}
						else if (tc_strcmp(DMLtype,"TODR")==0)
						{
							ITK_CALL(GRM_find_relation_type("CMReferences",&reln_type_tag));
						}
									...
									if(st_count1==1)
									{
										...
										if (tc_strcmp(WSO_Name,"T5_LcsErcRlzd")==0 || tc_strcmp(WSO_Name,"ERC Released")==0)
										{
											...
											if (effectivityStr!=NULL)
											{
												...
												//DATE_date_to_string(Rel_Date,"%d-%b-%Y %H:%M", &old_to_date_Rel_date);
												month++;
												if (month<10)
												{
												...
												if (day<10)
												{
													fprintf(DML_Download_fp,"/0%d/%d",day,year);
												}
												else
												{
													fprintf(DML_Download_fp,"/%d/%d",day,year);
												}
...
static void print_ATTR (tag_t reva,FILE *fpAssy)
 {
   ...
   if( AOM_ask_value_string(reva,"t5_PartType",&partType));
	...
	//if ( AOM_ask_value_string(reva,"t5_PunMakeBuyIndicator",&t5PunMakeBuyIndS));//Change PNR make buy
	//if ( AOM_ask_value_string(reva,"t5_PunIntialAgency",&t5PunIntialAgS));//change PNR Initial Agency
	...
	fprintf (fpAssy,"%s^%.3f^%s^%s^%s^%s^%s^\n",CompCode,dweight,Materialclass,t5EnvelopeDimenS,t5RolledupWtS,mcreator,VerCreator);
...
//	}
	...
	else
	{
		...
		//if(tc_strcmp(type_name2,"Design Revision")==0) // commented to get Child EE part in parent BOM 
		//{
			fprintf(fus,parent);
			fprintf(fus,",");
